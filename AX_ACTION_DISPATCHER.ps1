...
$runtimeUrl = $env:AX_CLOUDFLARE_RUNTIME_URL

if (-not [string]::IsNullOrWhiteSpace($runtimeUrl)) {
  Write-Host '=== CLOUDFLARE CONTROLLED DISPATCH ==='
  try {
    $health = Invoke-RestMethod -Uri "$($runtimeUrl.TrimEnd('/'))/health" -Method Get -TimeoutSec 10
    $runtimeHealthy = (
      $health.status -eq 'ONLINE' -and
      $health.mode -eq 'FREE_ONLY' -and
      $health.liveFinancialExecution -eq $false
    )
  ...
  if ($runtimeHealthy) {
    & powershell.exe -ExecutionPolicy Bypass -File "$PSScriptRoot\AX_CLOUDFLARE_DISPATCH_ADAPTER.ps1" `
      -TaskId $selected.id `
    ...
    Write-Host 'Cloudflare Runtime: NOT_READY — controlled execution deferred'
  }
} else {
