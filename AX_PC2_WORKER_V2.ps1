...
function Log([string]$m) {
  $line = "$(Get-Date -AsUTC -Format o) [$NodeId] $m"
  Add-Content -Path $LogPath -Value $line
  Write-Host $line
}

function Save-State([string]$status,[string]$jobId='', [int]$attempt=0, $payload=$null) {
  $state = [ordered]@{ protocol='AX PC2 WORKER v2'; nodeId=$NodeId; status=$status; currentJobId=$jobId; currentAttempt=$attempt; currentPayload=$payload; liveFinancialExecution=$false; updatedAt=(Get-Date -AsUTC -Format o) }
  $tmp = "$StatePath.tmp"
...
function Post-Node($body) {
  ...
  if ($r.success -ne $true) { throw ("NODE_API_REJECTED:" + ($r.error ?? $r.result.reason ?? 'UNKNOWN')) }
  return $r.result
}
...
function Execute-Safe($payload, [string]$jobId) {
  ...
  $record = [ordered]@{ protocol='AX-PC2-WORKER-V2'; jobId=$jobId; nodeId=$NodeId; command=$command; executed=$true; verified=$false; liveFinancialExecution=$false; timestamp=(Get-Date -AsUTC -Format o) }
