# AX CONTINUOUS REFLECTION LAYER
...
AX Continuous Reflection is a structured operational self-review layer. It is not human consciousness and it does not replace authoritative evidence or the AX MASTER BRAIN.

Its purpose is to continuously inspect what AX believes, what AX has executed, what evidence exists, what remains uncertain, what changed, and what should be learned before continuing.
...
2. **Claim audit** — Did AX claim an action or result without evidence?
...
- Verified Master Brain state outranks model-local memory.
...
- periodically while AX is active;
...
- whenever AX detects a material contradiction or uncertainty.

Periodic reflection is a safety net, not proof that work occurred.

## Relationship to Save Points

`AX CURRENT STATE → REFLECTION → AUTO-SAVE ENGINE → VERIFIED SAVE POINT`
...
7. It does not allow model-local memory to outrank authoritative Master Brain state.

## Parallel-track rule

Phase 1 Master Brain acceptance remains an independent hard gate:

`NEW CHAT → LOAD MASTER BRAIN → REHYDRATE AX → MASTER COMPANY REPORT → 12 TASKS + STATUS + TIMESTAMP → EVIDENCE/VERIFICATION → CONTINUE`
