...
[pscustomobject]@{protocol='AX PC1 FOREGROUND ROOT-CAUSE SNAPSHOT v2';nodeId='PC1';computerName=$env:COMPUTERNAME;timestampUtc=[DateTime]::UtcNow.ToString('o');terminalProcesses=$trace;serviceOwners=$services;interestingTasks=$interesting}|ConvertTo-Json -Depth 10|Set-Content (Join-Path $ev 'pc1-foreground-rootcause.json') -Encoding UTF8
