...
2. AERIS is removed from the AKATH company structure and has no authority, runtime role, business-unit role, or source-of-truth role in AKATH.
3. The previous AERIS-era blueprints are not used as AKATH blueprints. Their company structure and operational rules are retired. Only validated lessons learned may be re-authored as AKATH-native knowledge; the legacy blueprints themselves are not dependencies of the new system.
...
## 12. AERIS Removal / Legacy Handling

AERIS is explicitly excluded from the AKATH target architecture.

During implementation, AERIS company structure, AERIS-specific blueprints, AERIS runtime assumptions, AERIS-specific naming, and AERIS operational dependencies are to be removed from the active AKATH system rather than reused as architecture.

The old material is not an AKATH knowledge source by default. Only individual lessons that are independently validated may be rewritten into AKATH-native knowledge records. The legacy blueprint itself does not remain as an active dependency.

The AKATH target architecture must not depend on AERIS naming, AERIS-specific folder taxonomy, AERIS company assumptions, or AERIS runtime endpoints.
...
- AERIS systems have been safely migrated.
...
9. AERIS dependencies are removed from the target architecture and explicitly tracked for cleanup during implementation.
...
11. Remove active AERIS runtime/deployment dependencies and legacy business structure according to an explicit verified cleanup plan.
...
- The active company architecture no longer depends on AERIS.
