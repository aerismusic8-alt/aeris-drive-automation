...
export function claimNextEligibleJob(registry,now=new Date()){
  for(const item of registry.tasks){
    if(item.status==='PENDING' && item.deadline_at && new Date(item.deadline_at)<=now && !item.retry_at){
      transitionJob(item,'OVERDUE',{overdue_at:now.toISOString()});
    }
  }
  ...
  const currentEligible=currentTask&&(!currentTask.retry_at||new Date(currentTask.retry_at)<=now)&&(!currentTask.deadline_at||new Date(currentTask.deadline_at)>now)?currentTask:null;
...
export function transitionJob(job,nextStatus,metadata={}){
  ...
  Object.assign(job,metadata,{status:nextStatus}); return job;
}
