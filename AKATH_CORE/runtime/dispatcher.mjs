...
function postJson(url,payload,timeoutMs=30000){
  return new Promise((resolve,reject)=>{
    ...
    const req=request({hostname:target.hostname,port:target.port||80,path:target.pathname+target.search,method:'POST',headers:{'content-type':'application/json','content-length:Buffer.byteLength(body)}},res=>{
      let out=''; res.setEncoding('utf8'); res.on('data',d=>out+=d); res.on('end',()=>{
        try { const parsed=JSON.parse(out); if(res.statusCode>=200&&res.statusCode<300) resolve(parsed); else reject(new Error(`NODE_HTTP_${res.statusCode}: ${out}`)); }
        catch { reject(new Error(`NODE_INVALID_JSON: ${out}`)); }
      });
    });
...
export function createAxNodeDispatcher({localAdapter=createLocalPc1Adapter(),nodeUrls={}}={}){
  return async function dispatch(job){
    ...
    if(node.nodeId==='PC1-MAIN' && node.transport==='local'){
      ...
      try {
        ...
        await heartbeatNode('PC1-MAIN',{activeJob:null,lastVerifiedJob:job.task_id});
        return result;
      } catch(error) {
        await heartbeatNode('PC1-MAIN',{activeJob:null,status:'DEGRADED',lastError:error.message});
        throw error;
      }
    }
    const base=nodeUrls[node.nodeId] || process.env[`AX_${node.nodeId.replaceAll('-','_')}_URL`];
    if(!base) throw new Error(`NODE_TRANSPORT_NOT_CONFIGURED: ${node.nodeId}`);
    await heartbeatNode(node.nodeId,{activeJob:job.task_id});
    try {
      const result=await postJson(new URL('/dispatch',base).toString(),{protocol:'AX-NODE/1',nodeId:node.nodeId,job});
      await heartbeatNode(node.nodeId,{activeJob:null,lastVerifiedJob:job.task_id});
      return result;
    } catch(error) {
      await heartbeatNode(node.nodeId,{activeJob:null,status:'DEGRADED',lastError:error.message});
      throw error;
    }
  };
}
