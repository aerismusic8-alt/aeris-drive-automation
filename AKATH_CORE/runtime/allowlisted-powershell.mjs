...
const ACTIONS = Object.freeze({
  ...
  youtube_short_package: Object.freeze({
    script: '$root = if ($env:AX_PC2_REVENUE_OUTPUT) { $env:AX_PC2_REVENUE_OUTPUT } else { Join-Path $PSScriptRoot "AERIS_REVENUE_OUTPUT" }; New-Item -ItemType Directory -Force -Path $root | Out-Null; $taskId = if ($env:AX_PC2_REVENUE_TASK_ID) { $env:AX_PC2_REVENUE_TASK_ID } else { "PC2-REV-YT-SHORT-$([DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())" }; $title = if ($env:AX_PC2_REVENUE_TITLE) { $env:AX_PC2_REVENUE_TITLE } else { "AERIS Music — YouTube Short" }; $manifest = [ordered]@{taskId=$taskId;channel="AERISMusicTH";type="youtube_short_package";title=$title;createdAt=[DateTime]::UtcNow.ToString("o");status="READY_FOR_QC";autonomous=$true}; $manifest | ConvertTo-Json -Depth 10 | Set-Content -Path (Join-Path $root "manifest.json") -Encoding UTF8; $metadata = [ordered]@{channel="AERISMusicTH";title=$title;description="AERIS MUSIC autonomous short package";tags=@("AERISMusicTH","AERIS MUSIC","AI Music")}; $metadata | ConvertTo-Json -Depth 10 | Set-Content -Path (Join-Path $root "metadata.json") -Encoding UTF8; $scriptText = if ($env:AX_PC2_REVENUE_SCRIPT) { $env:AX_PC2_REVENUE_SCRIPT } else { "Create a concise AERIS MUSIC short from the approved source material. Verify title, metadata, and rights before publication." }; Set-Content -Path (Join-Path $root "script.txt") -Value $scriptText -Encoding UTF8; Write-Output "[REVENUE] OUTPUT=$root"; Write-Output "[REVENUE] RESULT=YOUTUBE_SHORT_PACKAGE_READY"'
  })
});
