...
$launcher = Join-Path $PSScriptRoot 'start-ax-runtime.ps1'
...
Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Description 'AERIS AKATH continuous PC1 runtime. K explicitly controls shutdown.' -Force | Out-Null
Write-Host "[AX_INSTALL] $taskName registered"
Write-Host "[AX_INSTALL] launcher=$launcher"
Write-Host '[AX_INSTALL] PC1 launcher binds the local executor automatically from the installed Node.js command.'
