...
const MAX_DIRECT_ATTACHMENT_BYTES = 25 * 1024 * 1024;

function json(body: unknown, status = 200): Response {
  return Response.json(body, { headers: { 'Cache-Control': 'no-store', 'Content-Type': 'application/json; charset=utf-8' }, status });
}
function constantTimeEqual(left: string, right: string): boolean {
  const a = new TextEncoder().encode(left), b = new TextEncoder().encode(right);
  if (a.length !== b.length) return false; let diff = 0; for (let i = 0; i < a.length; i += 1) diff |= a[i] ^ b[i]; return diff === 0;
}
...
function geminiAccounts(env: AxAdaptersEnv): GeminiAccount[] {
  return [env.GEMINI_ACCOUNT_1, env.GEMINI_ACCOUNT_2, env.GEMINI_ACCOUNT_3].map((apiKey, index) => apiKey ? { id: `G${index + 1}`, apiKey } : null).filter((account): account is GeminiAccount => Boolean(account));
}
...
async function uploadDirectAttachment(request: Request, env: AxAdaptersEnv): Promise<Response> {
  if (!authorized(request, env)) return json({ error: 'AUTH_REQUIRED' }, 401);
  const missionId = (request.headers.get('X-AX-Mission-Id') || '').trim();
  const attachmentId = (request.headers.get('X-AX-Attachment-Id') || `ATT-${crypto.randomUUID()}`).trim();
  const name = (request.headers.get('X-AX-Attachment-Name') || 'attachment').trim();
  const mediaType = (request.headers.get('Content-Type') || 'application/octet-stream').trim();
  if (!missionId || !name) return json({ error: 'ATTACHMENT_SCHEMA_INVALID' }, 422);
  if (!mediaType.startsWith('image/') && !mediaType.startsWith('video/')) return json({ error: 'ATTACHMENT_MEDIA_TYPE_NOT_ALLOWED' }, 422);
  if (Number(request.headers.get('Content-Length') || '0') > MAX_DIRECT_ATTACHMENT_BYTES) return json({ error: 'ATTACHMENT_TOO_LARGE', maxBytes: MAX_DIRECT_ATTACHMENT_BYTES }, 413);
  const stub = directLedgerStub(env, missionId);
  if (!stub) return json({ error: 'DIRECT_AX_LEDGER_UNAVAILABLE' }, 503);
  const response = await stub.fetch(`https://mission.local/attachment?mission_id=${encodeURIComponent(missionId)}&attachment_id=${encodeURIComponent(attachmentId)}&name=${encodeURIComponent(name)}&media_type=${encodeURIComponent(mediaType)}`, { method: 'POST', headers: { 'Content-Type': mediaType }, body: request.body });
  return json(await response.json().catch(() => ({ error: 'ATTACHMENT_RESPONSE_INVALID' })), response.status);
}
async function directInput(request: Request, env: AxAdaptersEnv): Promise<Response> {
  ...
  if (attachments.some(item => !item || typeof item !== 'object' || !item.attachment_id)) return json({ error: 'ATTACHMENT_REFERENCE_INVALID' }, 422);
  const commandBase: Omit<MissionCommand, 'fingerprint' | 'created_at'> = { mission_id: missionId, task_id: taskId, request_id: requestId, content_type: contentType as MissionCommand['content_type'], content, attachments };
  ...
  const stub = directLedgerStub(env, missionId)!;
  const ledgerResponse = await stub.fetch('https://mission.local/put', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(mission) });
  ...
  if (kind === 'CREATED') {
    await env.AX_EXECUTION_QUEUE.send({ id: requestId, taskId, domain: 'AERIS', priority: Number(body.priority ?? 100), action: content || '', createdAt: now, source: 'AX_DIRECT_CHANNEL', attachments });
...
async function directMission(request: Request, env: AxAdaptersEnv, missionId: string): Promise<Response> {
  if (!authorized(request, env)) return json({ error: 'AUTH_REQUIRED' }, 401);
  const stub = directLedgerStub(env, missionId); if (!stub) return json({ error: 'DIRECT_AX_LEDGER_UNAVAILABLE' }, 503);
  return stub.fetch(`https://mission.local/get?task_id=${encodeURIComponent((new URL(request.url)).searchParams.get('task_id') || '')}`, { method: 'GET' });
}

export async function handleAxAdaptersRoute(request: Request, env: AxAdaptersEnv, dependencies: Dependencies = {}): Promise<Response | null> {
  ...
  if (url.pathname === '/ax/direct/attachment' && request.method === 'POST') return uploadDirectAttachment(request, env);
  if (url.pathname.startsWith('/ax/direct/mission/') && request.method === 'GET') { const missionId = decodeURIComponent(url.pathname.slice('/ax/direct/mission/'.length)).trim(); if (!missionId) return json({ error: 'MISSION_ID_REQUIRED' }, 422); return directMission(request, env, missionId); }
  if (!url.pathname.startsWith('/ax/')) return null;
  if (!authorized(request, env)) return json({ error: 'AUTH_REQUIRED' }, 401);
  if (request.method === 'GET' && url.pathname === '/ax/adapters/status') return json({ gemini: { configuredAccounts: geminiAccounts(env).length }, drive: { configured: Boolean(env.GOOGLE_DRIVE_CREDENTIAL) } });
  if (request.method === 'POST' && url.pathname === '/ax/gemini/generate') {
    let body: { prompt?: unknown }; try { body = await request.json() as { prompt?: unknown }; } catch { return json({ error: 'INVALID_JSON' }, 400); }
    const prompt = typeof body.prompt === 'string' ? body.prompt.trim() : ''; if (!prompt) return json({ error: 'PROMPT_REQUIRED' }, 400); if (prompt.length > 8_000) return json({ error: 'PROMPT_TOO_LARGE' }, 413);
    const router = new GeminiRouter(geminiAccounts(env), dependencies.generateGemini); const result = await router.generate(prompt); return json(result, result.ok ? 200 : 503);
  }
  if (request.method === 'GET' && url.pathname === '/ax/drive/list') { const query = (url.searchParams.get('q') || '').trim(); if (query.length > 200) return json({ error: 'DRIVE_QUERY_TOO_LARGE' }, 413); const adapter = new DriveAdapter(env.GOOGLE_DRIVE_CREDENTIAL || '', dependencies.driveFetch || fetch); const result = await adapter.list(query); return json(result, result.ok ? 200 : 503); }
