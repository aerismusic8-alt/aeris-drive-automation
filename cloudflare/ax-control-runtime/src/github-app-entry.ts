...
function wrapPkcs1AsPkcs8(pkcs1: Uint8Array): Uint8Array {
  ...
  // SEQUENCE { version INTEGER 0, rsaEncryption AlgorithmIdentifier, privateKey OCTET STRING }
...
async function createAppJwt(env: GitHubEnv): Promise<string> {
  ...
  const parsed = pemToDer(privateKeyPem);
  const keyDer = parsed.format === 'pkcs1' ? wrapPkcs1AsPkcs8(parsed.der) : parsed.der;
