...
function showAuthenticated() { $('authForm').classList.add('hidden'); $('controls').classList.remove('hidden'); $('chatPanel').classList.remove('hidden'); }
function showContext(data, tasks) {
  $('session').textContent = data.session_status || 'UNKNOWN'; $('identity').textContent = data.identity || '—'; $('rehydration').textContent = data.rehydration_status || '—';
  $('context').textContent = JSON.stringify({source_of_truth:data.source_of_truth,state_version:data.state_version,caller_identity:data.caller_identity,model_independence:data.model_independence,task_count:Array.isArray(tasks?.tasks)?tasks.tasks.length:null},null,2);
}
async function hydrate() {
  ...
  const [state, tasks] = await Promise.all([api('/gateway/state'), api('/gateway/tasks')]);
  showContext({...session,...state}, tasks); showAuthenticated();
}
$('login').onclick = async () => {
  try { const result = await api('/auth/login',{method:'POST',body:JSON.stringify({username:$('username').value,password:$('password').value})}); setAuth(result.token); await hydrate(); }
  catch (error) { $('session').textContent = error.message; }
};
$('refresh').onclick = async () => { try { const session=await api('/gateway/session'); const [state,tasks]=await Promise.all([api('/gateway/state'),api('/gateway/tasks')]); showContext({...session,...state},tasks); } catch (error) { $('session').textContent=error.message; } };
...
$('send').onclick = async () => {
  try {
    ...
    $('messages').textContent += `\n\n[${result.status}] ${result.request_id}\n${result.content || ''}`; $('result').textContent=JSON.stringify(result,null,2); $('message').value=''; $('file').value='';
