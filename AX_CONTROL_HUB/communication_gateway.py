...
_CAPABILITIES = {
    'K': {'read_state', 'read_tasks', 'read_evidence', 'submit_input', 'm_a_check'},
    'SERVICE': {'read_state', 'read_tasks', 'read_evidence', 'submit_input', 'm_a_check'},
    'READ_ONLY': {'read_state', 'read_tasks', 'read_evidence', 'm_a_check'},
}
...
class CommunicationGateway:
    ...
    def create_session(self, auth_token: str) -> dict[str, Any]:
        identity = self._auth(auth_token); challenge = self.store.challenge(); state = self.store.read_state()
        ready = bool(challenge.get('verified') and challenge.get('source') == 'A_MASTER_BRAIN')
    ...
    def get_capabilities(self, auth_token: str) -> dict[str, Any]:
        identity = self._auth(auth_token)
        return {'identity': identity, 'capabilities': sorted(_CAPABILITIES[identity]), 'source_of_truth': 'A_MASTER_BRAIN', 'model_identity_is_ax_identity': False}
    ...
    def handle_input(self, payload: dict[str, Any], auth_token: str) -> dict[str, Any]:
        ...
        request_id = str(payload.get('request_id') or secrets.token_urlsafe(16)); idempotency_key = str(payload.get('idempotency_key') or '').strip()
        ...
        record = {'request_id': request_id, 'task_id': str(payload.get('task_id') or secrets.token_urlsafe(12)), 'idempotency_key': idempotency_key, 'status': 'RECEIVED', 'source_channel': str(payload['source_channel']), 'content_type': str(payload['content_type']), 'content': payload.get('content'), 'rehydration_status': challenge['status'], 'evidence_status': 'PENDING', 'verification_status': 'PENDING', 'attachments': [dict(item) for item in payload.get('attachments', [])], 'source_of_truth': 'A_MASTER_BRAIN', 'transport_store': 'GATEWAY_INPUT_INBOX', 'runtime_profile': challenge.get('runtime_profile')}
        try: self.input_queue.put(record)
        except ValueError as exc:
            if str(exc) == 'REQUEST_ID_CONFLICT': raise ValueError('REQUEST_ID_CONFLICT') from exc
            raise
        return record
