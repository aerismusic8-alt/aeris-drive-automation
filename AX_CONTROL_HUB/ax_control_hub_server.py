...
class MasterBrainStore:
    ...
    def write_command_audit(self,request_id,idempotency_key,actor,verification_status,expected_version=None):
        with self._lock:
            ...
            if verification_status == 'VERIFIED': new_state['last_verified_evidence']={'request_id':request_id,'verification_status':'VERIFIED','source':'AX_CONTROL_HUB'}
...
class AuthStore:
    ...
    def login(self,username,password):
        ...
        token=secrets.token_urlsafe(32); self.sessions[token]={'username':username,'created':time.time(),'identity':'K'}; return token
    def valid(self,token): return self.role(token) is not None
    def role(self,token):
        item=self.sessions.get(token)
        if item is not None and time.time()-item['created']<3600: return item['identity']
        service=os.getenv('AX_CONTROL_HUB_SERVICE_TOKEN','')
        readonly=os.getenv('AX_CONTROL_HUB_READ_ONLY_TOKEN','')
        if service and hmac.compare_digest(token,service): return 'SERVICE'
        if readonly and hmac.compare_digest(token,readonly): return 'READ_ONLY'
        return None
...
class CommandLedger:
    ...
    def recover_pending(self,store):
        for request_id, record in list(self.records.items()):
            ...
            else: transition=store.write_command_audit(record['request_id'],record['idempotency_key'],record['actor'],'VERIFIED')
            recovered_state=store.read_state()
            if recovered_state.get('last_verified_evidence',{}).get('request_id')!=request_id: store.write_command_audit(record['request_id'],record['idempotency_key'],record['actor'],'VERIFIED',int(recovered_state.get('state_version',0)))
...
GATEWAY=CommunicationGateway(STORE,LEDGER,AUTH.valid,INPUT_QUEUE,identity_resolver=AUTH.role)

def json_bytes(obj): return json.dumps(obj,ensure_ascii=False).encode('utf-8')

class Handler(BaseHTTPRequestHandler):
    server_version='AXControlHub/1.0'
    ...
    def body(self):
        ...
        if length>1024*1024: raise ValueError('REQUEST_TOO_LARGE')
    ...
    def protected(self):
        identity=AUTH.role(self.token())
        if identity is None: self.send_json(401,{'error_code':'AUTH_REQUIRED'}); return False
        return True
    ...
    def do_GET(self):
        ...
        token=self.token()
        try:
            if path in {'/state','/gateway/state'}: self.send_json(200,GATEWAY.get_state(token))
            elif path in {'/tasks','/gateway/tasks'}: self.send_json(200,GATEWAY.get_tasks(token))
            elif path=='/gateway/session': self.send_json(200,GATEWAY.create_session(token))
            elif path=='/gateway/capabilities': self.send_json(200,GATEWAY.get_capabilities(token))
            elif path.startswith('/evidence/') or path.startswith('/gateway/evidence/'):
                request_id=path.split('/')[-1]
                try: self.send_json(200,GATEWAY.get_evidence(request_id,token))
                ...
                try: self.send_json(200,GATEWAY.get_input(request_id,token))
        ...
        except PermissionError as exc: self.send_json(403,{'error_code':str(exc)})
    def do_POST(self):
        path=urlparse(self.path).path
        try: data=self.body()
        except Exception as exc: self.send_json(400,{'error_code':str(exc) or 'INVALID_REQUEST'}); return
        if path=='/auth/login':
            ...
            self.send_json(200,{'authenticated':True,'token':token,'identity':'K'}); return
        if not self.protected(): return
        token=self.token()
        ...
        if path=='/gateway/input':
            ...
            except RuntimeError as exc: self.send_json(503,{'error_code':str(exc)})
            except ValueError as exc:
                code=str(exc); status=409 if code in {'REQUEST_ID_CONFLICT','DUPLICATE_IDEMPOTENCY_KEY'} else 400
                self.send_json(status,{'error_code':code})
            return
        if path=='/command':
            ...
            if AUTH.role(token)!='K': self.send_json(403,{'error_code':'CAPABILITY_DENIED'}); return
            ...
            except ValueError as exc: LEDGER.release(idem,request_id); self.send_json(409,{'error_code':str(exc)}); return
            except (OSError,json.JSONDecodeError): LEDGER.release(idem,request_id); self.send_json(503,{'error_code':'SOURCE_STATE_UNAVAILABLE'}); return
            record=LEDGER.get(request_id)
            if record is not None: record['state_writeback']=transition; record['lifecycle_status']='COMPLETED'; LEDGER._persist(record)
