...
def wait_health(base: str) -> None:
    for _ in range(50):
        try:
            ...
            if status == 200 and payload.get("health_status") == "HEALTHY":
                return
        except Exception:
            time.sleep(0.1)
...
def run_profile(tmp: Path, port: int, profile: str) -> tuple[subprocess.Popen[str], dict, dict, dict]:
    state = tmp / f"{profile}-AX_MASTER_STATE.json"
    tasks = tmp / f"{profile}-AX_MASTER_TASK_REGISTRY_v2.json"
    contract = tmp / f"{profile}-AX_REHYDRATION_ADAPTER_SPEC.md"
    ...
    env = os.environ.copy()
    env.update({
        "AX_CONTROL_HUB_HOST": "127.0.0.1",
        "AX_CONTROL_HUB_PORT": str(port),
        "AX_MASTER_BRAIN_DIR": str(tmp),
        "AX_MASTER_STATE_PATH": str(state),
        "AX_MASTER_TASK_REGISTRY_PATH": str(tasks),
        "AX_CONTROL_HUB_USERNAME": "portability-test",
        "AX_CONTROL_HUB_PASSWORD": "portability-test-password",
        "AX_RUNTIME_PROFILE": profile,
    })
    ...
    payloads = (state_payload, tasks_payload, challenge)
...
def main() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            for state_payload, tasks_payload, challenge in (
                (state_a, tasks_a, challenge_a),
                (state_b, tasks_b, challenge_b),
            ):
