...
def main():
    with tempfile.TemporaryDirectory() as td:
        ...
        state_path=master/"AX_MASTER_STATE.json"
        ...
        env=os.environ.copy(); env.update({"AX_MASTER_BRAIN_DIR":str(master),"AX_MASTER_STATE_PATH":str(state_path),"AX_MASTER_TASK_REGISTRY_PATH":str(master/"AX_MASTER_TASK_REGISTRY_v2.json"),"AX_CONTROL_HUB_EVIDENCE_DIR":str(evidence),"AX_CONTROL_HUB_USERNAME":"k","AX_CONTROL_HUB_PASSWORD":"test-password","AX_CONTROL_HUB_PORT":"8792"})
        ...
        try:
            ...
            assert after.get("state_version")==1
            ...
            stale={"request_id":"state-writeback-stale","idempotency_key":"state-writeback-stale-idem","actor":"K","command":"health_check","args":{},"requested_at":"2026-09-03T00:00:01Z","expected_state_version":0}
            status,conflict=request(base,"POST","/command",stale,token)
            assert status==409 and conflict["error_code"]=="STATE_VERSION_CONFLICT"
            assert json.loads(state_path.read_text()).get("state_version")==1

            tampered=json.loads(state_path.read_text()); tampered["identity_authority"]="MODEL_LOCAL"
            state_path.write_text(json.dumps(tampered),encoding="utf-8")
            protected={"request_id":"state-writeback-protected","idempotency_key":"state-writeback-protected-idem","actor":"K","command":"health_check","args":{},"requested_at":"2026-09-03T00:00:02Z"}
